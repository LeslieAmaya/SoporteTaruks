...
const Ekkipo = () => {
  ...
  const [idEkkipo, setIdEkkipo] = useState(null);
  ...
  const fetchSistemas = async () => {
    try {
      ...
      const ekkipoSistema = response.data.find((sistema) => sistema.nombreSis === "Ekkipo");
      if (ekkipoSistema) {
        setIdEkkipo(ekkipoSistema.idSis);
        console.log("ID de Ekkipo encontrado:", ekkipoSistema.idSis);
      } else {
        console.warn("No se encontró el sistema 'Ekkipo'");
      }
    } catch (error) {
  ...
  const fetchModulos = async () => {
    try {
      ...
      const modulosEkkipo = response.data.filter(
        (modulo) => Number(modulo.idSis) === Number(idEkkipo)
      );

      console.log("Módulos filtrados para Ekkipo:", modulosEkkipo);
      setModulos(modulosEkkipo);
    } catch (error) {
  ...
  <div className="guia-details ml-4 mt-4">
    ...
    <p><strong>Procedimiento:</strong> 
      <div dangerouslySetInnerHTML={{ __html: selectedGuia.procedimiento }}></div>
    </p>
  </div>
)}
